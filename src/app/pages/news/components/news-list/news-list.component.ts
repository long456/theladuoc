...
export class NewsListComponent implements OnInit{
  ...
  ngOnInit() {
    ...
    .pipe(
      ...
      mergeMap(([page, pageSize, filter]) => {
        return this.newsService.getNewsList(page, pageSize, filter)
          .pipe(
            map((value) => {
              return {
                rows: value.data.articleList,
            ...
            catchError(err => {
              this.message.error('Lỗi load dữ liệu danh sách tin tức');
              return of({
                rows: [],
                page: 0,
                pageSize: 0,
  ...
  create():void {
    this.router.navigate(['/page/news/create']);
  }

  edit(data: any):void {
    this.router.navigate(['/page/news/' + data.id]);
  }
  ...
  delete():void {
    if (this.itemSelectList.length === 0) {
      ...
      this.modal.confirm({
        ...
        nzOnOk: () => {
          this.newsService.softDeleteNews(this.itemSelectList)
            .pipe(
            ).subscribe({
            next: value => {
              if (value.success) {
