...
import { MembershipCourseModelResponse } from '../../model/course-elearning-reponse';
...
@Component({
  selector: 'app-report-member-ship-course',
  templateUrl: './report-member-ship-course.component.html',
  styleUrls: ['./report-member-ship-course.component.scss']
})
export class ReportMemberShipCourseComponent {
  dateFormat = 'dd/MM/yyyy';
  monthFormat = 'MM/yyyy';
  items: MembershipCourseModelResponse[] = [];
  ...
  openListStudentsDialog(countStudents?: number, courseElearningId?: number,
    ...
    let title = "Số người đăng ký khóa học thành viên"
    if (numberOfLearners100Percent == true) {
      title = "Số học viên hoàn thành khóa học thành viên 100%"
    } else if (numberOfLearners90Percent == true) {
      title = "Số học viên hoàn thành khóa học thành viên 90%"
    }
    else if (numberOfLearners75Percent == true) {
      title = "Số học viên hoàn thành khóa học thành viên 75%"
    }
    else if (numberOfLearners50Percent == true) {
      title = "Số học viên hoàn thành khóa học thành viên 50%"
    }
    else if (numberOfLearners25Percent == true) {
      title = "Số học viên hoàn thành khóa học thành viên 25%"
    }
    else if (numberOfLearners5Percent == true) {
      title = "Số học viên hoàn thành khóa học thành viên 5%"
    }
  ...
  openSearch() {
    ...
    this.reportService.getReportMembershipCourses(this.currentPage, this.pageSize, this.filter).subscribe(x => {
      if (x) {
        this.items = x.data.reportMembershipCourse.map((z: any) => plainToClass(MembershipCourseModelResponse, z));
  ...
  openFilter() {
    ...
    const modal: NzModalRef = this.modal.create<FilterElearningCourseReportComponent>({
      nzTitle: 'Tìm kiếm khóa học thành viên',
      nzContent: FilterElearningCourseReportComponent,
      nzFooter: null,
      nzWidth: '35%',
      nzData: {
        filter: this.filter,
        courseType: ElearningType.MEMBER,
  ...
  syncData() {
    ...
    this.reportService.syncElearningCourseData(ElearningType.MEMBER).pipe(finalize(() => {
