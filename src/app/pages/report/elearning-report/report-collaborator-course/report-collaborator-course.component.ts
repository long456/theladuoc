...
import { CollaboratorModelResponse } from '../../model/course-elearning-reponse';
...
@Component({
  selector: 'app-report-collaborator-course',
  templateUrl: './report-collaborator-course.component.html',
  styleUrls: ['./report-collaborator-course.component.scss']
})
export class ReportCollaboratorCourseComponent {
  dateFormat = 'dd/MM/yyyy';
  monthFormat = 'MM/yyyy';
  items: CollaboratorModelResponse[] = [];
  ...
  openListStudentsDialog(countStudents?: number, courseElearningId?: number,
    ...
    let title = "Số người đăng ký khóa học cộng tác viên"
    if (numberOfLearners100Percent == true) {
      title = "Số học viên hoàn thành khóa học cộng tác viên 100%"
    } else if (numberOfLearners90Percent == true) {
      title = "Số học viên hoàn thành khóa học cộng tác viên 90%"
    }
    else if (numberOfLearners75Percent == true) {
      title = "Số học viên hoàn thành khóa học cộng tác viên 75%"
    }
    else if (numberOfLearners50Percent == true) {
      title = "Số học viên hoàn thành khóa học cộng tác viên 50%"
    }
    else if (numberOfLearners25Percent == true) {
      title = "Số học viên hoàn thành khóa học cộng tác viên 25%"
    }
    else if (numberOfLearners5Percent == true) {
      title = "Số học viên hoàn thành khóa học cộng tác viên 5%"
    }
  ...
  openSearch() {
    ...
    this.reportService.getReportCollaboratorCourses(this.currentPage, this.pageSize, this.filter).subscribe(x => {
      if (x) {
        this.items = x.data.reportCollaboratorCourse.map((z: any) => plainToClass(CollaboratorModelResponse, z));
  ...
  openFilter() {
    ...
    const modal: NzModalRef = this.modal.create<FilterElearningCourseReportComponent>({
      nzTitle: 'Tìm kiếm khóa học cộng tác viên',
      nzContent: FilterElearningCourseReportComponent,
      nzFooter: null,
      nzWidth: '35%',
      nzData: {
        filter: this.filter,
        courseType: ElearningType.COLLAB,
  ...
  syncData() {
    ...
    this.reportService.syncElearningCourseData(ElearningType.COLLAB).pipe(finalize(() => {
