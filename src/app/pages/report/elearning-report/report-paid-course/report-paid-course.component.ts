...
import { PaidCourseModelResponse } from '../../model/course-elearning-reponse';
...
@Component({
  ...
  templateUrl: './report-paid-course.component.html',
  styleUrls: ['./report-paid-course.component.scss']
})
export class ReportPaidCourseComponent {
  ...
  openSearch() {
    ...
    this.reportService.getReportPaidCourses(this.currentPage, this.pageSize, this.filter).subscribe(x => {
      if (x) {
        this.items = x.data.reportPaidCourse.map((z: any) => plainToClass(PaidCourseModelResponse, z));
  ...
  openFilter() {
    ...
    const modal: NzModalRef = this.modal.create<FilterElearningCourseReportComponent>({
      nzTitle: 'Tìm kiếm khóa học trả phí',
      nzContent: FilterElearningCourseReportComponent,
      nzFooter: null,
      nzWidth: '35%',
      nzData: {
        filter: this.filter,
        courseType: ElearningType.PAID,
  ...
  syncData() {
    ...
    this.reportService.syncElearningCourseData(ElearningType.PAID).pipe(finalize(() => {
